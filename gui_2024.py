...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.window = parent
        self.controller = controller
        self.label = tk.Label(self, text= "Where is the Therapist looking to?", font=BOLD_LARGE_FONT)
        self.label.place(relx=0.32,rely=0.1)

        self.counter = -1
        self.filedirend = '\\therapistdata.txt'
        ...
        button = tk.Button(self, height = 1, width= 7, text=robot,font=BOLD_LARGE_FONT,bg="blue",
                            command=lambda: self.write_file(robot))
        button.place(relx=0.87,rely=0.20)
        self.controller.bind('t',self.write_file(robot))

        button2 = tk.Button(self, height = 1, width= 7, text=altra_p,font=BOLD_LARGE_FONT,bg="red",
                            command=lambda: self.write_file(altra_p))
        button2.place(relx=0.87,rely=0.30)
        self.controller.bind('b',self.write_file(altra_p))

        button3 = tk.Button(self, height = 1, width= 7, text=poster_dx,font=BOLD_LARGE_FONT,bg="red",
                            command=lambda: self.write_file(poster_dx))
        button3.place(relx=0.87,rely=0.40)
        self.controller.bind('b',self.write_file(poster_dx))


        button5 = tk.Button(self, height = 1, width= 7, text=poster_sx,font=BOLD_LARGE_FONT,bg="red",
                            command=lambda: self.write_file(poster_sx))
        button5.place(relx=0.87,rely=0.50)
        self.controller.bind('b',self.write_file(poster_sx))


        button6 = tk.Button(self, height = 1, width= 7, text=poster_dietro,font=BOLD_LARGE_FONT,bg="red",
                            command=lambda: self.write_file(poster_dietro))
        button6.place(relx=0.87,rely=0.60)
        self.controller.bind('b',self.write_file(poster_dietro))

        button7 = tk.Button(self, height = 1, width= 7, text=toy,font=BOLD_LARGE_FONT,bg="red",
                            command=lambda: self.write_file(toy))
        button7.place(relx=0.87,rely=0.70)
        self.controller.bind('b',self.write_file(toy))
        ...
        button4 = tk.Button(self, height = 1, width= 7, text=nowhere,font=BOLD_LARGE_FONT,bg="yellow",
                            command=lambda: self.write_file(nowhere))
        button4.place(relx=0.87,rely=0.80)
        self.controller.bind('a',self.write_file(nowhere))
    ...
    def write_file(self, text):
        if self.controller.v_reader is not None:
            ...
            print(rate)
            if self.counter >= -1:
                if text == 'Patient' and self.filedirend == '\\therapistdata.txt':
                    text = 'Patient'
                elif text == 'Patient' and self.filedirend == '\\patientdata.txt':
                    text = 'Therapist'
                ...
                f.close()
            if self.counter + rate < self.controller.vidlen:
                if self.counter == -1:
                    self.counter = 0
                else:     
                    self.counter = self.counter + rate
                ...
                # otherwise, simply update the panel
                    ...
                    self.panel.image = imagePI
            elif ~self.controller.patient:
                print('passei aqui')
                # self.controller.on_closing()
                self.controller.v_reader.set_image_index(0)
                self.counter = 0
                self.label.configure(text = "Where is the Patient looking to?")
                self.controller.patient = True
                self.filedirend = '\\patientdata.txt'
            else:
